...
// 	lib:            dynlib.Dynlib,
// 	lib_path:       string,
// 	lib_updated_at: time.Time,
// 	version:        int,
// }

// PluginRegistry :: struct {
// 	plugins: [dynamic]Plugin,
// 	symbols: map[typeid]rawptr,
// 	stage:   core.Stage,
// }

// plugin_registry_setup :: proc(registry: ^PluginRegistry) -> (ok: bool) {

// 	if registry.stage != .UNINITIALIZED {
// 		log.warn("Plugin registry is already initialized.");return
// 	}

// 	registry.plugins = make([dynamic]Plugin)
// 	registry.symbols = make(map[typeid]rawptr)
...
plugin_system_load_all :: proc() {
	...
	for pf in plugin_folders {
		...
		for pff in pf_files {
			if pff.type == .Regular && filepath.ext(pff.name) == ".dll" && filepath.stem(pff.name) == pf.name {
				p := Plugin {
